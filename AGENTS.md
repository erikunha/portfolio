...
| **Before `speckit-plan` on any spec** | **`thinking-risk-premortem` — enumerate the class-of-bugs the implementation introduces; each becomes an explicit plan task, not a claude-review finding. ENFORCED at this trigger only: `.codex/hooks/mandated-skill-gate.sh` (PreToolUse `Skill`) blocks `speckit-plan` until a `thinking-risk-premortem` tool_use exists this session. The row above (write-time) stays prose — no mechanical event names it** |
...
| Before any UI code review (the Playwright MCP visual check) | `web-design-guidelines` — ENFORCED: `.codex/hooks/mandated-skill-gate.sh` blocks `mcp__plugin_playwright_playwright__browser_navigate` until the skill has run this session. Anchored to navigation because that is the one mechanical event a visual check cannot skip; the other browser tools stay ungated so the block fires once per session, not per interaction |
