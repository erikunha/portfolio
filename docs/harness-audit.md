...
The parser-based guard closes every statically-analyzable invocation form (six review rounds' worth: substitution, compounds, redirects, wrappers, interpreters incl. `-c`/`eval` re-parse, global flags, force-flag forms, brace expansion, `find -exec`, here-strings). The convergence floor is **runtime expansion**: `x=npm; $x i` (variable indirection) and `base64 -d <<<… | sh` (decode-then-exec) require actually *evaluating* shell state, which no static analyzer (parser included) can do. This is accepted: `bash-guard` is a cooperative guardrail, these forms are deliberate obfuscation no cooperative agent emits by accident, and the two irreversible ops it guards (force-push / direct-push to `main`, `gh pr merge`) are gated server-side (branch protection, owner-only merge) regardless. Recorded justified in the findings ledger.
