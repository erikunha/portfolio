...
2. **Do NOT re-request after a push — the push already triggers a review.**
   ...
   `gh pr view <n> --json reviewThreads,statusCheckRollup` plus `pnpm claude-gate`
   show which case applies; check before commenting rather than by reflex. The
   state command `pnpm review:converge` was removed 2026-07-27; this skill is the
   procedure and you run it yourself.
...
10. Repeat 5-9 until CI is green AND `pnpm pr:gate` reports 0 unresolved threads
   AND `pnpm claude-gate` exits OK (it gates on a claude[bot] **Approve** verdict
   that is non-stale — reviewed SHA == HEAD, fail-closed on an Approve naming no
   SHA). Only then tell the repo owner to run `gh pr merge`.
