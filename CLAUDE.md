...
- **Copilot convergence loop — re-request after every push AND after every thread resolution batch.** On an open PR, the full loop is: (1) push → re-request Copilot immediately; (2) poll CI until green; (3) check for new Copilot threads (`gh api graphql` with `reviewThreads(first:100)`); (4) if threads exist → fix code (commit) OR reply-and-resolve (stale/already-fixed) → re-request Copilot after EITHER action; (5) wait for Copilot review to complete; (6) repeat steps 3–5 until CI green AND Copilot review returns 0 unresolved threads AND `pnpm ready-to-merge` exits OK; (7) only then tell the repo owner to run `gh pr merge`. Never declare "ready to merge" before Copilot has completed a review with 0 new threads after the last action.
