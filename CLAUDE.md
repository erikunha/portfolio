...
- **Stopping coding work always orchestrates the full review suite.** Whenever coding work stops — finishing a branch, declaring a task done, ending a session on a feature — run `pr-review-toolkit:review-pr` (dispatches review subagents in parallel) BEFORE transitioning. For `finishing-a-development-branch` specifically: run it before presenting push/PR options. Never wait for the user to ask — the review is mandatory, not optional.
