...
When about to merge a PR: rebase first (non-dependabot, unless the already-reviewed exception applies), THEN check readiness on the post-rebase HEAD: `pnpm claude-gate` for the claude-review Approve, `gh pr view <pr> --json reviewThreads` for every thread resolved and carrying a reply, and `pnpm ci:local`. AI agents must never call `gh pr merge` — the bash-guard blocks it (exit 2); the repo owner runs the final merge once converge reports converged.
