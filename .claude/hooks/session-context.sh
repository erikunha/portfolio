...
# unaware CI is red" error class. `gh` calls are bounded by `timeout 5` WHEN
# `timeout` is present (stock macOS has none, in which case gh runs unbounded
# rather than being skipped) and fall back to git-only on any failure /
# unauthenticated gh. Outside a git repo: emits nothing. ALWAYS exits 0 — a
# SessionStart hook must never error the session.
...
# Optional time bound: stock macOS ships no GNU `timeout`. When absent, run `gh`
# unbounded rather than skipping the enrichment entirely (the old `timeout 5 gh`
# form failed wholesale on macOS-without-coreutils, silently degrading to git-only).
