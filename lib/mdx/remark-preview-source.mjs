// Remark plugin — injects `source` prop into <Preview> elements at the MDAST stage.
// Remark runs before JSX is compiled, so the VFile still holds the raw MDX source text.
// We slice the child content using AST position offsets and attach it as a prop.
