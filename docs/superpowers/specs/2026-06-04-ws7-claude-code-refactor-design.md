...
| `.claude/skills/ai-eval-update/SKILL.md` | Procedure for maintaining the ask:eval harness post-WS2/WS3: when to add corpus entries (`content/ask-eval-corpus.ts`), running `pnpm ask:eval`, interpreting the correctness and jailbreak-resistance thresholds, reading `ask:eval:latest` from Upstash Redis, the judge-calibration gate added in WS3, and when to re-request CI after corpus changes. Under 200 lines. |
