...
7. **Copilot auto-reviews on PR open; reply to threads + re-request after every review-feedback push.** Copilot reviews a PR automatically on *open* — do NOT post any comment on open. After any push that addresses Copilot or reviewer feedback: (a) reply to each resolved thread via `gh api repos/erikunha/portfolio/pulls/<pr>/comments/<databaseId>/replies -f body="Fixed in <sha>. <explanation>"` — start every reply with the fix SHA and a one-sentence technical reason; (b) re-request the review: `gh pr edit <pr> --add-reviewer copilot-pull-request-reviewer`. No additional PR-level comment needed — the re-request alone triggers Copilot. The raw REST `requested_reviewers` API and GraphQL `requestReviews` mutation both reject Copilot as `not a collaborator`; `gh pr edit` uses a different mechanism that works.
