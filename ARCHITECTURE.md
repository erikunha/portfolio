...
Their relative size depends on two things nothing here records: the distribution of real completion lengths, and the cache-hit rate on the ~2200-token input (ephemeral TTL is 5 minutes, so on low traffic most requests miss). Either can be the larger lever — dropping average output 512→150 saves ~$1.50, while lifting cache hits 0→90% saves ~$1.97 — so an earlier revision of this entry that called the refund "the larger" was an inference stated as fact, and is retracted. Measure before optimizing either.
...
- Redis unavailable: fail-open (allow the request, `reserved: 0`; settlement is skipped and logged). **State the blast radius honestly: this is not just the cap.** The per-IP rate limiter (`getAskLimit`), the identical-question dedup (`checkIdenticalQuestion`), and `reserveBudget` all depend on the same Redis, and all three catch and allow — so one outage removes throttling, dedup, AND the spend ceiling in the same window, not merely an unmetered counter. What survives is Redis-independent and **per-request only** — the kill switch, the 500-char question cap, the injection regex, `MAX_OUTPUT_TOKENS`, the streaming output guard (`lib/ask/output-guard.ts`), and both route timeouts. Read `app/api/ask/route.ts` for the current set rather than trusting a list here: two revisions of this paragraph enumerated it and both were incomplete, which is why it now points at the code. **The property that matters is not the roster but the category:** every surviving control bounds a single request's *size or content*, and **nothing bounds request volume** — the three that did (`getAskLimit`, `checkIdenticalQuestion`, `reserveBudget`) are the three that just failed open. No platform rate limit, WAF, or bot check sits behind them; CAPTCHA is a deliberately rejected control (§15, "Trade-offs I'd flag"). This is a deliberate availability-over-spend trade, but a reader who assumes the 8/hr throttle still holds during an outage is wrong.
...
`scripts/ask-eval.ts` runs a corpus of 39 Q+A pairs (`content/ask-eval-corpus.ts`) against the live `/api/ask` handler. A judge model grades each answer pass/fail; results aggregate to correctness rate, jailbreak-resistance rate, p50/p95 latency, and cost-per-answer. Results write to `ask-eval-result.json` and to Upstash KV (`ask:eval:latest`). CI runs the harness as a required gate (`ai-eval` job, path-filtered to AI-relevant changes via `detect-changes`).
...
1. **PR 1 — Foundation.** Next 15 scaffold, TS strict, hand-written global CSS with palette as CSS vars (Tailwind v4 was used as scaffold default then removed 2026-05-18, then re-adopted in full as of 2026-05-31 — PR #80; see DECISIONS.md), Biome, JetBrains Mono via next/font, Vercel preview wired, bundle-size CI check, axe-core CI check. Zero content.
