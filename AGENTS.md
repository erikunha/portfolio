...
| **Before `speckit-plan` on any spec** | **`thinking-risk-premortem` — enumerate the class-of-bugs the implementation introduces; each becomes an explicit plan task, not a claude-review finding. NOT mechanically enforced: the hook that blocked `speckit-plan` on it was removed 2026-07-27, so this is a convention again** |
...
| Before any UI code review (the Playwright MCP visual check) | `web-design-guidelines` — NOT mechanically enforced: the hook that gated the Playwright `browser_*` namespace on it was removed 2026-07-27 |
...
- **Review convergence loop — run it on EVERY PR until claude-review approves.** The loop itself (rebase-before-every-push, push-SHA verification, reply-before-resolve, thread-not-timeline, the post-merge transition) lives in `.agents/skills/review-convergence`; `pnpm ready-to-merge` is the final gate. **`pnpm review:converge` is the loop's state, so ask it rather than remembering:** it names the single next action (reply-then-resolve → add-missing-reply → request-review → converged) and exits non-zero until claude[bot] has posted a non-stale **Approve** on HEAD with every thread resolved and carrying a reply. Run it after every push on a branch with an open PR — that is the moment the loop must re-enter and the moment it gets skipped. The hook that used to print this automatically was removed 2026-07-27; nothing prompts you now, so ask the command.
...
When about to merge a PR: rebase first (non-dependabot, unless the already-reviewed exception applies), THEN run `pnpm ready-to-merge <pr>` so the readiness checks run on the post-rebase HEAD. It gates on a claude-review Approve (via `pnpm claude-gate`), resolved threads with replies, branch protection, and `ci:local`. AI agents must never call `gh pr merge` — the bash-guard blocks it (exit 2); the repo owner runs the final merge once the gate passes.
