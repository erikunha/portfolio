...
The mix is the whole point, because 3M tokens is $3.00 if all input and $15.00 if all output. The **ceiling** assumes every request burns its full 2712-token reservation (2200 input + 512 output): ~1106 requests, ~2.43M input + ~0.57M output, ≈$5.3. The **floor** is $3.00, approached as real completions shrink. Real spend sits in between, nearer the floor, for two reasons — and the first is the larger:
...
- Redis unavailable: fail-open (allow the request, `reserved: 0`; settlement is skipped and logged). **State the blast radius honestly: this is not just the cap.** The per-IP rate limiter (`getAskLimit`), the identical-question dedup (`checkIdenticalQuestion`), and `reserveBudget` all depend on the same Redis, and all three catch and allow — so one outage removes throttling, dedup, AND the spend ceiling in the same window, not merely an unmetered counter. What survives is per-request only: `MAX_OUTPUT_TOKENS` (512), the 500-char question cap (`q.trim().slice(0, 500)`), the injection regex, the kill switch, and the request timeout — all Redis-independent. Every one of them bounds a request's *size or content*; **nothing bounds request volume**, because the three controls that did are the three that just failed open. There is no platform rate limit, WAF, or bot check behind them (CAPTCHA is a deliberately rejected control — see "Anti-spam strategy"). This is a deliberate availability-over-spend trade, but a reader who assumes the 8/hr throttle still holds during an outage is wrong.
