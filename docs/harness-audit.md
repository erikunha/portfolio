...
The parser-based guard closes every statically-analyzable invocation form (six review rounds' worth: substitution, compounds, redirects, wrappers, interpreters incl. `-c`/`eval` re-parse, global flags, force-flag forms, brace expansion, `find -exec`, here-strings). The convergence floor is **deliberate obfuscation** in two classes a static analyzer cannot close: (1) **runtime expansion** — `x=npm; $x i` (variable indirection), `base64 -d <<<… | sh` (decode-then-exec) — which require *evaluating* shell state; (2) **multi-level nested-quote escaping** — `bash -c 'bash -c '"'"'npm i'"'"''` — which the vendored bashlex 0.18 mis-dequotes at the second level (a parser limitation, not the detector's logic). Both are accepted: `bash-guard` is a cooperative guardrail, none of these forms is anything a cooperative agent emits by accident, and the two irreversible ops it guards (force-push / direct-push to `main`, `gh pr merge`) are gated server-side (branch protection, owner-only merge) regardless. Recorded justified in the findings ledger. The guard survived 8 adversarial review rounds; every realistic invocation form (substitution, compounds, redirects, wrappers, bundled/attached interpreter flags, brace expansion, find -exec, here-strings, aliases, trailing-slash pathspecs) is closed.
