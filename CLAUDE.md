...
- **Decide visual-baseline impact BEFORE every push.** Determine whether the change touches a section captured in `tests/visual/visual.spec.ts` (hero, contact, shell, hottest-takes, DS components) and state the assessment. If NO, push once. If YES, regenerate and commit BOTH-platform baselines in the same commit — full darwin/linux procedure (`--update-snapshots`, the `update_visual_baselines` CI dispatch, artifact download, per-project `-linux.png` copy) in `.claude/skills/visual-baseline-regen`. Always inspect every regenerated PNG before committing; batch tweaks to one push; never let a paid CI run be your baseline-detection tool.
