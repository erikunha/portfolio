...
| **copilot-convergence** | Converging an open PR's review to green | The loop: rebase before every push, verify the pushed SHA, re-request Copilot after each push, reply-before-resolve, the PR-comment-gate timing race, the automatic post-merge transition. Not for the final merge. |
