...
| Before writing plans | Before invoking `speckit-plan` on any spec | `architect-reviewer` — `.claude/hooks/architect-gate.sh` (PreToolUse `Skill`) is WIRED to block `speckit-plan` unless the transcript shows an `architect-reviewer` `GATE_RESULT: PASS` (in a tool_result) this session. **Confirmed enforced (2026-06-06):** Skill matcher fired exit-2 and blocked `speckit-plan` in a live session without a prior `GATE_RESULT: PASS`. Boundary: session-scoped PASS, not per-spec identity |
...
- Before invoking `speckit-plan`, dispatch `architect-reviewer` against the spec. It runs the four-gate spec-gate protocol and must return `GATE_RESULT: PASS` before `speckit-plan` proceeds. `.claude/hooks/architect-gate.sh` (PreToolUse `Skill` matcher) is WIRED to block (`exit 2`) the `speckit-plan` invocation unless the transcript shows an `architect-reviewer` `GATE_RESULT: PASS` (scoped to a tool_result block, so prose quoting the sentinel cannot spoof it) this session. **Confirmed enforced (2026-06-06):** Skill matcher fired exit-2 and blocked `speckit-plan` in a live session without a prior `GATE_RESULT: PASS`. Boundaries: session-scoped PASS, not per-spec identity (spec identity is not a structured transcript field).
