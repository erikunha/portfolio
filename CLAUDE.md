...
- **Quality gates measure real properties — fix the property, not the gate.** When a gate fails (Lighthouse audit, Biome lint, bundle-size, a11y, security), identify the specific mechanism that caused the failure and fix it at the root. Acceptable responses: reduce the measured metric (LCP, TBT, bundle size, contrast ratio), change code to eliminate the violation, or fix a misconfigured assertion that fires on a valid state (e.g., `maxNumericValue` on an audit that returns null when there are zero violations). Unacceptable responses: set the audit to `"off"`, lower the threshold, add the rule to an ignore list, or wrap code to hide it from the checker. No exceptions. If a gate cannot be fixed within the current scope, escalate with a written explanation of the root cause and the minimum required change — do not merge with known suppression.
