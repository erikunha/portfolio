...
| Before any UI code review (the Playwright MCP visual check) | `web-design-guidelines` — ENFORCED: `.claude/hooks/mandated-skill-gate.sh` blocks the Playwright page-reaching tools (`browser_navigate`, `browser_tabs`) AND the observation sinks (`browser_take_screenshot`, `browser_snapshot`) until the skill has run this session. Reaching a page is an open set (`browser_evaluate` and `browser_run_code_unsafe` also get there), so the sink is what the mandate binds to: whatever route reached the page, looking at it is the reviewable act. Tools that neither reach nor observe (console reads) stay ungated, so the block fires once per session |
