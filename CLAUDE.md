...
7. **Copilot auto-reviews on PR open; re-request after every review-feedback push.** Copilot reviews a PR automatically on *open* — do NOT post any comment on open. After any push that addresses Copilot or reviewer feedback, re-request the review with: `gh pr edit <pr> --add-reviewer copilot-pull-request-reviewer`. This uses the `gh` CLI reviewer-request mechanism (not the raw REST API, which rejects Copilot as `not a collaborator`). No additional comment needed — the re-request alone triggers Copilot.
