...
| 13-17 | `bash-guard.sh` used raw-substring greps, so it both (a) false-blocked a dangerous string appearing only inside a quoted argument like a commit message (13, hit live), and (b) was evaded by shell-tokenization-equivalent forms — all **verified** live once the extraction fix made the guard functional: quoted / double-spaced `gh "pr" merge` (14), quoted `git push --"force" origin main` (15), chained `npm install` (16), quoted `git add "-A"` / `git add "."` (17). A follow-up Opus+code-review battery then reproduced a further class the first tokenizer missed: no-space chaining (`x&&npm i`), newline-separated commands, command-position wrappers (`env`/`command`/`sudo`/`VAR=val npm`), and `git add :/`/`*` pathspecs. Mitigating context: `bash-guard` is a cooperative-agent guardrail, and the irreversible ops it names are *also* gated server-side (GitHub branch protection blocks force-push/direct-push to `main`; `gh pr merge` is owner-only) — the `git add`/`npm` blocks are the ones with no server-side backstop. A further Opus+code-review round proved the heuristic approach cannot converge: a `shlex`-based tokenizer still lost to command substitution (`` `npm i` ``), compound commands (`if npm; then`, `{ npm; }`), redirections (`>x npm`), wrapper option-grammars (`env -u X npm`), and `+main` refspecs — each needs a real grammar, not a pattern. **Fixed here** by vendoring the `bashlex` bash parser (`.claude/hooks/vendor/bashlex`, GPLv3+, pinned 0.18, dependency-free on py3.4+) and rewriting `.claude/hooks/bash-guard-detect.py` to walk the AST and inspect the *name of every command node* (including those inside substitutions/compounds/pipelines), with wrapper-arg scanning for `env`/`sudo`/… and a coarse grep fallback only if python3/bashlex is unavailable. The parser distinguishes command from argument, so it also stops over-blocking legit `grep npm` / `git log --grep npm` / push-to-`chore/maintain-docs` that a token heuristic would flag. Regression tests cover every reproduced bypass and the non-block cases | security-hardening | yes | done |

### bash-guard: accepted residual

The parser-based guard closes every statically-analyzable invocation form, each hardened across successive review rounds (the per-round history is the `git log` for `.claude/hooks/bash-guard-detect.py`, not a count reproduced here that would drift): substitution, compounds, redirects, wrappers, path-prefixed binaries (`/usr/bin/npm`), interpreters incl. `-c`/`eval` re-parse with bundled/attached flags (`python3 -Bc`), global flags, force-flag forms, brace expansion, `find -exec`, here-strings (incl. behind a path-prefix or a *bare* wrapper like `sudo bash`). The convergence floor is **deliberate obfuscation** in two classes a static analyzer cannot close: (1) **runtime expansion** — `x=npm; $x i` (variable indirection), `base64 -d <<<… | sh` (decode-then-exec) — which require *evaluating* shell state; (2) **multi-level nested-quote escaping** — `bash -c 'bash -c '"'"'npm i'"'"''` — which the vendored bashlex 0.18 mis-dequotes at the second level (a parser limitation, not the detector's logic). A third form is closable-but-deliberately-uncontained: a here-string body behind an **option-argument wrapper** (`timeout 5 sh <<< …`, `env -u X sh <<< …`) — `effective_program` resolves the interpreter past bare wrappers and `VAR=val` assignments but not each wrapper's individual option grammar, so the positional/flag argument is mistaken for the program; the same `npm` sink is already reachable through the explicitly-allowed `bash script.sh`, so encoding per-wrapper flag grammars buys no marginal containment. All are accepted: `bash-guard` is a cooperative guardrail, none of these forms is anything a cooperative agent emits by accident, and the two irreversible ops it guards (force-push / direct-push to `main`, `gh pr merge`) are gated server-side (branch protection, owner-only merge) regardless. Recorded justified in the findings ledger. Every realistic invocation form (substitution, compounds, redirects, wrappers, path-prefixed binaries, bundled/attached interpreter flags, brace expansion, find -exec, here-strings, aliases, trailing-slash pathspecs) is closed; the per-round convergence history lives in the commit log, not a count reproduced here.
