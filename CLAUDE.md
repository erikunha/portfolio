...
| Gate | Trigger | Agent |
|---|---|---|
| Before writing plans | Before invoking `speckit-plan` on any spec | `architect-reviewer` — convention, not enforced: the hook that blocked `speckit-plan` on a `GATE_RESULT: PASS` was removed 2026-07-27 |
| After API changes | After editing `app/api/`, `lib/rate-limit.ts`, or `proxy.ts` | `security-auditor` — convention, not enforced: the PostToolUse marker and the push guard were removed 2026-07-27 |
...
| **Before `speckit-plan` on any spec** | **`thinking-risk-premortem` — enumerate the class-of-bugs the implementation introduces; each becomes an explicit plan task, not a defect found after the fact. NOT mechanically enforced: the hook that blocked `speckit-plan` on it was removed 2026-07-27, so this is a convention again** |
...
- **Tag gates with evidence.** A gate should record when it last fired and what it caught (see the gate rows above). A gate that has neither fired nor false-fired in ~90 days is unproven — audit whether it still earns its place.
...
- **Review your own diff before opening the PR.** Read `git diff origin/main...HEAD` end to end and invoke `pr-review-toolkit:code-reviewer` against it (`review-pr` is a slash command, not an agent). Fix every Critical/Important finding before `gh pr create`. There is no mechanical gate on this: the review battery, its findings ledger, the dispatch stamp and the pre-push stamp check were removed 2026-07-27 and stay removed. claude-review reviews the PR once opened; this pre-PR pass is discipline, not enforcement.
...
- **Review convergence loop — run it on EVERY PR until claude-review approves.** claude-review (claude[bot]) runs automatically on open and on every push and posts one inline thread per finding. Each round: rebase (`git fetch && git rebase origin/main`) BEFORE the push, push, verify the pushed SHA equals the PR head, poll the run to completion (a job reporting `success` may have posted nothing — check for a `claude[bot]` comment, never the job conclusion), fix, then **reply in the thread citing the fix SHA before resolving it**. Do NOT comment `/claude-review` after a push: the push already triggered a run and the comment lands in the same per-PR concurrency group and cancels it; re-request only when no run will otherwise fire (a completed run that posted no verdict, or a PR editing `claude-review.yml`, where the action loads the workflow from the PR head and refuses). Done when CI is green, `pnpm claude-gate` passes (non-stale Approve on HEAD), and `pnpm pr:gate` passes — it fails `silent_resolve` on any thread resolved without a reply. Full rationale: `docs/handbook/review-merge-release.md`.
- **Every PR comment goes in a review THREAD — never a timeline comment (owner rule, 2026-07-12).** `gh pr comment <N>` posts an unanchored `#issuecomment-…`: attached to no code, not resolvable, absent from `reviewThreads`, therefore invisible to `pnpm pr:gate`, which reads `reviewThreads`. Reply in the existing thread (`.../pulls/<N>/comments/<id>/replies`); if a finding has no thread — e.g. one your own review battery raised — **create** one on the file+line (`.../pulls/<N>/comments` with `commit_id`+`path`+`line`), do not fall back to a timeline comment. A self-raised finding that never became a thread is an unrecorded finding. The sole exception is the `/claude-review` re-request, which is a command, not a comment.
...
- **Playwright MCP visual check before writing or changing tests.** After implementing any section or component (or after changes that affect rendering), run `pnpm dev` and use the Playwright MCP tool to visually inspect desktop (1280×720) + mobile (375×812) BEFORE touching test files. Tests must assert observable, verified behavior — not assumed behavior. If the visual check reveals something unexpected, fix it first. Only then write or update tests.
- **The review should be boring.** If your own pre-PR read finds real bugs, the pre-implementation discipline failed. `thinking-risk-premortem` before writing and TDD during implementation are the actual defences, and with no reviewer behind them they are now the only ones.
- **Every plan must include a failure-mode checklist.** Run `thinking-risk-premortem` before `speckit-plan` on any task. Each bug class becomes an explicit plan task — not a defect found after the fact.
...
Slow (5m): `git revert HEAD && git push --no-verify`. (main push guard requires --no-verify for emergency revert; revert commit still goes through CI after landing, but has no PR and therefore no review.)
Verify: `curl https://erikunha.dev/api/healthz | jq .sha`

## PR merge gate

When about to merge a PR: rebase first (non-dependabot, unless the already-reviewed exception applies), THEN check readiness on the post-rebase HEAD: `pnpm ci:local`, and `pnpm pr:gate <pr>` for thread resolution (GraphQL `isResolved` on every thread, and it flags `suspicious_self_resolve`). AI agents must never call `gh pr merge`; the repo owner runs the final merge. Note that nothing blocks it mechanically now — the bash-guard that returned exit 2 was removed 2026-07-27.
...
- Before invoking `speckit-plan`, dispatch `architect-reviewer` against the spec and act on its four-gate verdict. Convention only: the hook that blocked `speckit-plan` on a `GATE_RESULT: PASS` was removed 2026-07-27.
