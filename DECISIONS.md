...
- **2026-05-18** · Critical-CSS inline render-blocking trade-off: the `<style>{CRITICAL_CSS}</style>` block in `app/layout.tsx` does NOT remove the render-blocking `globals.css` `<link>` — Next App Router's CSS pipeline always emits the import as render-blocking. The inline still provides a real LCP benefit by getting hero dimensions and tokens to the paint registry earlier in the HTML parse. Converting globals.css to async-load via `preload`/`onload` was rejected as too invasive (requires ejecting from Next's CSS pipeline). _Reversible: switch to async CSS load if Next adds first-class support or if the pipeline is ejected for another reason._
