...
export function HottestTakesSection({ defer }: { defer?: boolean } = {}) {
  return (
    <Module
      ...
      defer={defer}
    >
      <div className="text-primary-400 text-sm tracking-[0.06em] mb-[18px] pb-3 border-b border-dashed border-primary-quiet max-[768px]:text-xs">
      ...
      <ol className="list-none m-0 p-0" start={1} data-testid="hottest-takes-list">
        {hottestTakes.map((t) => (
          ...
          >
            <span className="text-primary-400 font-mono text-sm font-bold tracking-[0.04em] pt-0.5 max-[768px]:text-xs">
              {t.num}
            </span>
            <div className="flex flex-col gap-1.5">
              <p className="text-primary-500 font-bold tracking-[0.02em] m-0 leading-[1.5] max-[768px]:text-sm">
                <span className="inline-block text-primary-400 border border-primary-quiet px-1.5 mr-2 text-xs tracking-[0.14em] align-[1px] font-medium bg-transparent max-[768px]:text-xs">
                  {t.category}
                </span>
                {t.thesis}
              </p>
              <p className="text-tertiary-50 text-sm leading-[1.65] m-0 max-w-[740px] max-[768px]:text-sm">
                {t.body}
              </p>
            </div>
          </li>
