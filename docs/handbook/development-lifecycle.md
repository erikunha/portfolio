...
flowchart TD
    ...
    brainstorm --> spec["Spec: docs/superpowers/specs/YYYY-MM-DD-topic-design.md<br/>Context · Gaps to Close · Changes · Status: Approved"]
    spec --> agate{"architect-reviewer gate<br/>GATE_RESULT: PASS?"}
    agate -->|no| spec
    agate -->|yes| plan["Plan: docs/superpowers/plans/YYYY-MM-DD-topic.md<br/>decomposed tasks (often sharded into sub-PRs)"]
...
A design spec lands in `docs/superpowers/specs/` as `YYYY-MM-DD-<topic>-design.md`. Structure: `# Title` -> `**Date** / **Status: Approved**` -> `## Context` -> `## Gaps to Close` (numbered) -> `## Changes` (per file). The spec is the contract; it enumerates the gaps it closes. There are ~48 specs in the repo.
