...
| Before writing plans | Before invoking `writing-plans` on any spec | `architect-reviewer` — **enforced** by `.claude/hooks/architect-gate.sh` (PreToolUse `Skill`): blocks `superpowers:writing-plans` unless the transcript shows an `architect-reviewer` dispatch emitted `GATE_RESULT: PASS` this session. Boundary: session-scoped PASS, not per-spec identity |
...
- **Review before every push — no exceptions, mechanically enforced.** Before any `git push`, dispatch ALL 5 agents in parallel: `pr-review-toolkit:review-pr`, `accessibility-tester`, `security-auditor`, `performance-engineer`, `dependency-manager`. Then run `pnpm review:stamp`. **Enforced:** `scripts/review-stamp.ts` refuses to write `.review-passed` unless the session transcript shows all five `subagent_type` dispatches since the last commit (fail-closed: an unresolvable transcript also refuses; override the resolution with `REVIEW_STAMP_TRANSCRIPT=<abs path>`). `.husky/pre-push` then blocks unless the stamp matches HEAD; `.husky/post-commit` clears the stamp on every commit, forcing a fresh review. **Boundary (convention, not enforced):** "fix all Critical/Important findings" is your responsibility — the stamp proves the agents were DISPATCHED, not that their findings were resolved. Skipping an agent requires written justification in the commit message. Applies to direct-to-main and PR branches alike.
- **Runtime gates before any push touching non-docs files — enforced by `.husky/pre-push`.** After the full review battery passes: (1) run `pnpm ci:local` (unit tests + lint + typecheck + content + naming gates); (2) `.husky/pre-push` computes the push range and **runs `pnpm gates:runtime --skip-build`** (build + LHCI + axe + E2E functional) automatically when any changed file is outside the docs-only allowlist (`*.md`, `content/`, `docs/`). Docs-only pushes skip `gates:runtime` but still run `pnpm verify`. Escape hatch: `SKIP_RUNTIME_GATES=1 git push` bypasses deliberately and is logged loudly (CI runs the same gates as required jobs regardless).
...
- **No PR is opened until baselines + tests + performance have actually run — always, no exceptions.** For ANY visual-affecting change (color, layout, typography, spacing), regenerating visual-regression baselines is a mandatory pre-PR step, not an afterthought — a stale baseline fails the E2E visual project in CI (darwin local PROD-server `--update-snapshots`; linux via the CI baseline workflow per the regen procedure). **Never defer baselines, the test suite, or runtime/perf (LHCI) gates pending a design or shade decision** — "the shade might change" is not a reason to skip gates. Lock the decision, then run every gate, then open. If the decision is not locked, hold the PR; do not open a half-gated one.
- **Decide baseline impact BEFORE every push, and push the regenerated baselines in the same commit — paid CI runs are not a baseline-detection tool.** Before any push, determine whether the change touches a visual-regression baseline (a section captured in `tests/visual/visual.spec.ts` — hero, contact, shell, hottest-takes, DS components). State the assessment explicitly. If NO baseline is affected (e.g. a string moved to content with identical rendering), say so and push once. If YES: regenerate and commit baselines for BOTH platforms alongside the code so the first CI run is green. Darwin: local PROD-server `--update-snapshots`. Linux: regenerate via the `update_visual_baselines` CI dispatch (`gh workflow run "CI" -f update_visual_baselines=true --ref <branch>`), wait for it, download the `visual-baselines-<project>` artifacts, copy each project's OWN `-linux.png`, commit. (A local Docker generator is desirable to make this one push, but only if its pixels are first validated to match the CI runner — do not commit an unvalidated one; it churns review.) NEVER push code that knowingly leaves a stale baseline and let CI fail, then dispatch a regen, then push again — that is 3 paid runs (build + dual-LHCI ~9min + 4 visual jobs) for what should be 1. Batch visual tweaks so this whole cycle happens once. Always inspect regenerated baselines (Read the PNG) before committing; never blind-update.
...
3. **AI agents must RESOLVE or ESCALATE every open comment.** RESOLVE = fix commit + SHA reply + behavioral test update. ESCALATE = verbatim comment + 2-3 options + recommendation; wait for decision. No third bucket. **Every reply to a review thread MUST cite the specific commit SHA that addresses it** (`Fixed in <sha>. <reason>`) — a reply with no commit reference is not a resolution. If the resolution is a non-code change (PR description/metadata edit, or "won't fix" with rationale), still reference the commit(s) the decision relates to (or state explicitly that no commit was required and why), so the thread is auditable from the SHA alone.
...
- Before invoking `writing-plans`, dispatch `architect-reviewer` against the spec. It runs the four-gate spec-gate protocol and must return `GATE_RESULT: PASS` before `writing-plans` proceeds. **Enforced** by `.claude/hooks/architect-gate.sh` (PreToolUse `Skill` matcher): the `superpowers:writing-plans` invocation is blocked (`exit 2`) unless the transcript shows an `architect-reviewer` dispatch emitted `GATE_RESULT: PASS` this session. Boundary: the gate confirms architect ran and passed *this session*, not that it passed *this exact spec* (spec identity is not a structured transcript field).
