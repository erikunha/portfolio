...
> Auto-loaded by Codex every session in this repo. Keep it tight — verbosity costs token budget on every invocation.
...
|---|---|
| `pnpm review:stamp` | After ALL 4 review agents are dispatched AND the findings ledger is resolved. `scripts/review-stamp.ts` **refuses to write** `.review-passed` unless (a) the transcript shows all four battery agents dispatched since the branch's merge-base with `origin/main` — NOT since the last commit, which made every fix commit invalidate the battery that found the thing being fixed AND (b) `.review-findings.json` has no `open` Critical/Important finding (each `resolved` with a SHA or `justified` with a reason). Pre-push hook blocks until the stamp matches HEAD **on the pre-PR push only** — once the branch has an open PR the stamp check is skipped (claude-review owns post-PR review). Proves dispatch AND resolution |
| `pnpm review:findings` | Verification-loop ledger CLI (`add`/`resolve`/`justify`/`check`/`list`/`clear`). `battery-synthesis` records each Critical/Important finding; `check` and `review:stamp` refuse while any is `open`. See `.agents/skills/battery-synthesis` |
| `pnpm ready-for-pr` | Before `gh pr create` — runs ci:local + gates:runtime, prints next-step checklist |
...
| `pnpm ask:eval` | When maintaining the AI eval harness (corpus/calibration/runner changes) — calibration → corpus → gate, always writes `ask-eval-result.json`; writes `ask:eval:latest` to Upstash Redis only when `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` are set. See `.agents/skills/ai-eval-update` |
...
| Gate | Trigger | Agent |
|---|---|---|
| Before opening any PR | After all milestone commits, before `gh pr create` | `pr-review-toolkit:code-reviewer` — address all Critical/Important before opening (convention: opening the PR is not mechanically blocked). **`pr-review-toolkit:review-pr` is a slash COMMAND, not a dispatchable agent** — the plugin ships `commands/review-pr.md` and no `agents/review-pr.md`, so dispatching it as a `subagent_type` errors while still satisfying the role, which is why `BATTERY_ROLES` dropped it |
| Before writing plans | Before invoking `speckit-plan` on any spec | `architect-reviewer` — `.codex/hooks/architect-gate.sh` (PreToolUse `Skill`) is WIRED to block `speckit-plan` unless the transcript shows an `architect-reviewer` `GATE_RESULT: PASS` (in a tool_result) this session. **Confirmed enforced:** the 2026-06-06 live-session exit-2 was against the then-current `superpowers:writing-plans` token; the `speckit-plan` rename is held by the 8 architect-gate fixtures in the hook test suite (`pnpm test:hooks`), not by that older observation. Boundary: session-scoped PASS, not per-spec identity |
| After API changes | After editing `app/api/`, `lib/rate-limit.ts`, or `proxy.ts` | `security-auditor` — **enforced** by `.codex/hooks/api-edit-marker.sh` (PostToolUse records the edit) + `.codex/hooks/api-security-push-guard.sh` (PreToolUse blocks the next `git push` until a `security-auditor` dispatch follows the marker). Boundary: dispatch-scoped, blocks push not the edit |
...
Invoke the named skill inline (not as a subagent) before the described action. Project triggers below override global AGENTS.md when both apply to the same action.
...
| **Before `speckit-plan` on any spec** | **`thinking-risk-premortem` — enumerate the class-of-bugs the implementation introduces; each becomes an explicit plan task, not a claude-review finding. ENFORCED at this trigger only: `.codex/hooks/mandated-skill-gate.sh` (PreToolUse `Skill`) blocks `speckit-plan` until a `thinking-risk-premortem` call has **completed** this session. Completed, not merely emitted: a blocked or denied call still leaves its `tool_use` behind, so pairing to a non-error `tool_result` is what stops one throwaway attempt unlocking the mandate. The row above (write-time) stays prose — no mechanical event names it** |
...
**Hook authoring:** `.codex/hooks/*` PreToolUse guards that must BLOCK a tool call exit with code **2** (exit 1 is a non-blocking warning — the command still runs). `bash-guard.sh` blocks (broad `git add`, npm/yarn, `gh pr merge`, force-push, fallow non-read-only) use `exit 2`; verify a guard blocks with a live test, not its printed `[BLOCKED]` message.
...
- **Process feedback mid-workflow is a hard stop.** Pause immediately, incorporate into AGENTS.md and/or memory, confirm with the user, then resume.
- **Commit in scope blocks; merge by milestone.** Work accumulates in commits grouped by concern — one logical unit per commit (a component, a fix, a config change). Open a PR when a block is a natural milestone, judged by whether the change tells one reviewable story, not by a line count.
- **Large multi-part feature? Integration branch + sub-PRs.** When a feature is too big for one reasonable PR (the failure mode that bloated PR #81), branch an integration branch off main (`feat/<feature>`), then open small sub-PRs from `feat/<feature>-<part>` branches **into the integration branch** — each a reviewable milestone. When the feature is complete, open the `feat/<feature>` → main PR; it will be large by design, so note in the body that it was reviewed incrementally via the sub-PRs. For a standalone change, skip the integration branch and open a normal small PR straight to main per the milestone rule above.
- **Run the full 4-agent review battery ONCE, before opening the PR (the pre-PR gate) — never before every push.** Post-PR, the loop is claude-review only (see the review-convergence skill); the battery does not re-run on post-PR fix pushes. The pre-PR run: (1) Check what changed (`git diff`, `git status`). (2) Invoke `.agents/skills/review-battery` and dispatch all four roles in parallel per its table — it holds the reviewer prompts, the assembly order, the finding contract, the per-commit-type scoping, and the measured model routing. **Every dispatch sets `model` explicitly**: an omitted tier silently inherits the session model, and tier is the single biggest lever on what a reviewer finds (measured 2026-07-25: opus reviewers found 13/13 and 11/13 planted defects, sonnet 7/13 and 6/13, same prompts). (3) Run `pnpm review:stamp` immediately after dispatch, without blocking on agent completion. (4) Fix ALL Critical/Important findings before opening the PR. Never skip because you are confident. **Enforced:** `.husky/pre-push` requires a valid stamp (`.review-passed` matching HEAD) ONLY when the branch has no open PR — the pre-PR push. Once a PR is open (`gh pr list --head <branch> --state open` non-empty) the stamp check is skipped, because claude-review owns post-PR review; `pnpm verify` (the lint + typecheck + test chain — not `next build`, which is a CI-only gate via `pnpm ci`/`gates:runtime`) still runs on every non-deletion push, and the merge gate (`pnpm ready-to-merge`) still requires a claude-review Approve. `scripts/review-stamp.ts` refuses to write `.review-passed` unless the transcript shows all four `subagent_type` dispatches after the branch's merge-base with `origin/main` (NOT after HEAD — anchoring to HEAD made every fix commit invalidate the battery that found the thing being fixed, forcing a fresh 4-agent round per fix and contradicting the run-it-ONCE rule above; `REVIEW_STAMP_BASE` overrides the base ref for a sub-PR) (fail-closed: an unresolvable transcript also refuses; override with `REVIEW_STAMP_TRANSCRIPT=<abs path>`), and while any Critical/Important finding in `.review-findings.json` is `open` (record via `battery-synthesis` / `pnpm review:findings`). Direct pushes to main are blocked by the pre-push hook, so all changes go through a PR.
...
- **Review convergence loop — run it on EVERY PR until claude-review approves.** The loop itself (rebase-before-every-push, push-SHA verification, reply-before-resolve, thread-not-timeline, the post-merge transition) lives in `.agents/skills/review-convergence`; `pr-merge-gate` covers the final merge. **`pnpm review:converge` is the loop's state, so ask it rather than remembering:** it names the single next action (reply-then-resolve → add-missing-reply → request-review → converged) and exits non-zero until claude[bot] has posted a non-stale **Approve** on HEAD with every thread resolved and carrying a reply. `.codex/hooks/review-converge-reminder.sh` runs it automatically after any `git push` on a branch with an open PR, which is the moment the loop must re-enter and the moment it gets skipped. The hook is advisory by construction (PostToolUse cannot un-push, and `pnpm ready-to-merge` already gates the merge); its job is to make "the loop is not done" a line in the terminal instead of something to remember.
...
- **Auto-review before opening any PR.** Run `pnpm ready-for-pr` (ci:local + gates:runtime). Then invoke `pr-review-toolkit:code-reviewer` against the diff (`review-pr` is a slash command, not an agent). Address all Critical and Important findings before `gh pr create`. `pnpm ready-for-pr` is required even when the pre-push review has already run — it additionally covers `bundle-check`, `route-js-check`, and runtime gates. Use `--skip-runtime` only for docs-only PRs. Opening with known issues requires written justification in the PR body.
...
- **Playwright MCP visual check before writing or changing tests.** After implementing any section or component (or after changes that affect rendering), run `pnpm dev` and use the Playwright MCP tool to visually inspect desktop (1280×720) + mobile (375×812) BEFORE touching test files. Tests must assert observable, verified behavior — not assumed behavior. If the visual check reveals something unexpected, fix it first. Only then write or update tests. This also applies before the pre-merge Playwright check (see `.agents/skills/pr-merge-gate`).
- **The review should be boring.** If the review battery or claude-review finds real bugs, the pre-implementation discipline failed. `thinking-risk-premortem` before writing and TDD during implementation are the actual defences — not the review. Multi-round review cycles mean the writing process needs fixing.
- **Every plan must include a failure-mode checklist.** Run `thinking-risk-premortem` before `speckit-plan` on any task. Each bug class becomes an explicit plan task — not a claude-review finding after the fact.
- **When dispatching ANY subagent — implementer or review-only — always include in the prompt:** "Use `git add -u` or `git add <specific files>` — never `git add .`, `-A`, or `--all`. **Write no git state except staging and committing inside the tree you were dispatched into — that exception is the only one.** No moving any HEAD or index (`checkout`, `switch`, `reset`, `restore`, `bisect`), no ref writes (`update-ref`, `branch -f`, `rebase`, `cherry-pick`, `am`), no `stash` (the stack is repo-wide), no `git config` writes. Read another revision with `git show <sha>:<path>` or your own `git worktree add`." Phrase it as that one exception, never as a verb list — a list will miss `update-ref` and `git config`, which need no verb any list names. **Review-only agents need this most:** "don't touch files" does not forbid moving HEAD. Dispatch reviewers with `isolation: "worktree"` where you can; it isolates HEAD and index but not refs, stash, or config.
...
- **Verification before any completion claim.** Before reporting done, fixing, or passing: run `pnpm typecheck && pnpm test --run && pnpm build`, read the output, cite the result. "Should pass" is not evidence. If you catch yourself reasoning about whether it passes instead of running it, that is the tell: run it.
...
- `.codex/rules/*.md` — path-scoped rules that load only when editing matching files (keeps this file small); `api-boundary.md` covers `app/api/**` + server surface
...
- Before invoking `speckit-plan`, dispatch `architect-reviewer` against the spec. It runs the four-gate spec-gate protocol and must return `GATE_RESULT: PASS` before `speckit-plan` proceeds. `.codex/hooks/architect-gate.sh` (PreToolUse `Skill` matcher) is WIRED to block (`exit 2`) the `speckit-plan` invocation unless the transcript shows an `architect-reviewer` `GATE_RESULT: PASS` (scoped to a tool_result block, so prose quoting the sentinel cannot spoof it) this session. **Confirmed enforced:** the 2026-06-06 live-session exit-2 was against the then-current `superpowers:writing-plans` token; the `speckit-plan` rename is held by the 8 architect-gate fixtures in the hook test suite (`pnpm test:hooks`), not by that older observation. Boundaries: session-scoped PASS, not per-spec identity (spec identity is not a structured transcript field).
