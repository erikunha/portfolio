...
| Before any UI code review (the Playwright MCP visual check) | `web-design-guidelines` — ENFORCED: `.codex/hooks/mandated-skill-gate.sh` blocks Playwright `browser_navigate` **and** `browser_tabs` until the skill has run this session. Both, because `browser_tabs` takes a `url`: gating navigation alone left a first-party bypass (open the tab, screenshot it, never navigate). Tools that cannot reach a new page stay ungated, so the block fires once per session, not per interaction |
