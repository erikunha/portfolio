...
export function HeroBootAnimation({ variant }: Props) {
  ...
  useEffect(() => {
    ...
    const onFirstLoop =
      variant === 'desktop'
        ? () => {
            // Find the parent <section> via DOM traversal — works since bootRef
            // is mounted inside .hero--desktop > .hero__left.
    ...
    const ctrl = runBoot(el, specs, dialog, onFirstLoop ? { ...timing, onFirstLoop } : timing);
