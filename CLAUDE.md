...
- **Copilot convergence loop** — converging an open PR's review to green lives in `.claude/skills/copilot-convergence` (rebase-before-every-push, push-SHA verification, reply-before-resolve, the PR-comment-gate timing race, the automatic post-merge transition). Invoke it whenever working an open PR toward merge; `pr-merge-gate` covers the final merge.
