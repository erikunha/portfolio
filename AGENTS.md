...
| Before any UI code review (the Playwright MCP visual check) | `web-design-guidelines` — ENFORCED: `.codex/hooks/mandated-skill-gate.sh` blocks the Playwright page-reaching tools (`browser_navigate`, `browser_tabs`) AND the observation sinks (`browser_take_screenshot`, `browser_snapshot`) until the skill has run this session. Reaching a page is an open set (`browser_evaluate` and `browser_run_code_unsafe` also get there), so the sink is what the mandate binds to: whatever route reached the page, looking at it is the reviewable act. Tools that neither reach nor observe (console reads) stay ungated, so the block fires once per session |
...
- **The review should be boring.** If `pr-review-toolkit:review-pr` or claude-review finds real bugs, the pre-implementation discipline failed. `thinking-risk-premortem` before writing and TDD during implementation are the actual defences — not the review. Multi-round review cycles mean the writing process needs fixing.
