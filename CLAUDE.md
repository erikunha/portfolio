...
- **Copilot convergence loop — rebase on main first, then loop until green.** On an open PR, the full loop is: (0) `git fetch && git rebase origin/main` — always rebase before the first push of any loop session; (1) push → immediately verify GitHub HEAD matches local HEAD (`gh api repos/erikunha/portfolio/pulls/<N> --jq '.head.sha'` must equal `git rev-parse HEAD`; if not, the push silently failed — re-push before continuing); (2) re-request Copilot immediately after every successful push (`gh pr edit <N> --add-reviewer copilot-pull-request-reviewer`); (3) poll CI until green; (4) check for new Copilot threads (`gh api graphql` with `reviewThreads(first:100)`); (5) if threads exist → fix code (commit) OR reply-and-resolve (stale/already-fixed) → re-request Copilot after EITHER action; (6) wait for Copilot to complete new review; (7) repeat steps 4–6 until CI green AND Copilot review returns 0 unresolved threads AND `pnpm ready-to-merge` exits OK; (8) only then tell the repo owner to run `gh pr merge`. Never declare "ready to merge" before verifying push landed, CI is green, and Copilot has 0 new threads after the last re-request.
- **Always reply to a Copilot thread before (or instead of silently) resolving it.** Pattern: "Fixed in `<sha>`. `<what changed>` — `<why>`." A thread with only 1 comment (the Copilot comment) is a process gap — never resolve without a reply. Use `mcp__plugin_github_github__add_reply_to_pull_request_comment` (the `gh api .../replies` REST endpoint 404s on resolved threads). After any push, verify every resolved thread has ≥ 2 comments; retroactively add replies to any that don't.
