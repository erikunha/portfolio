...
- **AI usage.** The agent should use the mandated skills (brainstorming before features, TDD before implementation, systematic-debugging on bugs, verification-before-completion before claiming done). The review battery runs once before opening a PR (post-PR review is owned by claude-review).
