...
When about to merge a PR, invoke `.claude/skills/pr-merge-gate`: rebase first (non-dependabot, unless the already-reviewed exception applies), THEN run `pnpm ready-to-merge <pr>` so the readiness checks run on the post-rebase HEAD. The skill covers the full 10-point gate: claude-review Approve requirement (mechanical gate via `pnpm claude-gate`), resolve-thread ground truth, RESOLVE-or-ESCALATE with SHA-cited replies, in-session reviewer findings, self-resolve detection, the `ready-to-merge` command, branch-protection invariant, the claude-review re-request loop, the local Playwright visual check, and the rebase rule (with its dependabot + already-reviewed exceptions). AI agents must never call `gh pr merge` — the bash-guard blocks it (exit 2). Once all gate points pass, the repo owner executes the final merge command in an external terminal or via the GitHub UI.
