...
7. **Copilot auto-reviews on PR open; re-request after every review-feedback push.** Copilot reviews a PR automatically on *open* — do NOT post `@copilot /review` on open (creates duplicate). After any push that addresses Copilot or reviewer feedback, re-request the review with: `gh pr edit <pr> --add-reviewer copilot-pull-request-reviewer`. This uses the `gh` CLI reviewer-request mechanism (not the raw REST API, which rejects Copilot as `not a collaborator`). Copilot returns the review as a formal `reviews[]` object on the PR. When requesting re-review, also post a rich context comment explaining what changed, the quality gates to check, and a summary of changes since the prior review — do NOT post a bare `@copilot /review`.
