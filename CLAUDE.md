...
- **Auto-review before opening any PR.** Run `pnpm ci:local`, then `pnpm bundle-check` and `pnpm route-js-check`, then `pnpm gates:runtime` (skip the last three for docs-only PRs). Then invoke `pr-review-toolkit:code-reviewer` against the diff (`review-pr` is a slash command, not an agent). Address all Critical and Important findings before `gh pr create`. Opening with known issues requires written justification in the PR body.
...
- **Fill the PR template when creating PRs — hard gate.** Before writing a PR body: `cat .github/pull_request_template.md`, then fill EVERY section (Summary, Type of change, Test plan, Visual changes, Checklist). Never write a custom body from scratch — always start from the template structure. Re-read the created body against the template before requesting review: an empty section is a section you skipped.
...
When about to merge a PR: rebase first (non-dependabot, unless the already-reviewed exception applies), THEN check readiness on the post-rebase HEAD: `pnpm claude-gate` for the claude-review Approve, `pnpm pr:gate <pr>` for thread resolution (it is the mechanical check — GraphQL `isResolved` on every thread, and it flags `suspicious_self_resolve`, which a manual read cannot), and `pnpm ci:local`. AI agents must never call `gh pr merge` — the bash-guard blocks it (exit 2); the repo owner runs the final merge once those three pass.
...
- Before invoking `speckit-plan`, dispatch `architect-reviewer` against the spec. It runs the four-gate spec-gate protocol and must return `GATE_RESULT: PASS` before `speckit-plan` proceeds. `.claude/hooks/architect-gate.sh` (PreToolUse `Skill` matcher) is WIRED to block (`exit 2`) the `speckit-plan` invocation unless the transcript shows an `architect-reviewer` `GATE_RESULT: PASS` (scoped to a tool_result block, so prose quoting the sentinel cannot spoof it) this session. **Confirmed enforced:** the 2026-06-06 live-session exit-2 was against the then-current `superpowers:writing-plans` token; the `speckit-plan` rename is held by the 8 architect-gate fixtures in the hook test suite (`pnpm test:hooks`), not by that older observation. Boundaries: session-scoped PASS, not per-spec identity (spec identity is not a structured transcript field).
