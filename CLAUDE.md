...
- **Review convergence loop — run it on EVERY PR until claude-review approves.** The loop itself (rebase-before-every-push, push-SHA verification, poll-the-run, reply-before-resolve, thread-not-timeline, the post-merge transition) lives in `.claude/skills/review-convergence`. claude-review (claude[bot]) runs automatically on open and on every push and posts inline findings. Rebase before every push, verify the pushed SHA landed, reply in a thread citing the fix SHA before resolving it, and re-read CI. Do NOT comment `/claude-review` after a push: the push already triggered a run, and the comment lands in the same per-PR concurrency group and cancels it. Re-request only when no run will otherwise fire — a completed run that posted no parsable verdict, or a PR editing `claude-review.yml` itself, where the action loads the workflow from the PR head and refuses.
