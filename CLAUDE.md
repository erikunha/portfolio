...
- **`finishing-a-development-branch` always orchestrates the full review suite first.** When invoking `superpowers:finishing-a-development-branch`, run `pr-review-toolkit:review-pr` (which dispatches the review subagents in parallel) BEFORE presenting the push/PR options. Never present the options until the review is complete and all Critical/Important findings are resolved. Do not wait for the user to ask — the review is mandatory, not optional.
