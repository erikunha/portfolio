...
- **1a — Migrate `/api/ask` onto Vercel AI Gateway.** Replace the direct `new Anthropic()`
  ...
  cost tracking, and model fallback. **Constraint:** the budget-cap reservation/settle logic and
  the cache-hit-rate accounting are coupled to the direct SDK's `usage` fields
  (`input_tokens`, `cache_read_input_tokens`, `cache_creation_input_tokens`); the migration must
  preserve the budget cap and the cache metric — verify the Gateway exposes equivalent usage
  data, and adapt `lib/rate-limit.ts` `settleBudget` if the shape differs.
...
- **1c — Visible metrics.** The AI feature must *show* it is measured. Add an on-page metrics
  panel — a small section on `/` — surfacing eval pass-rate, cache-hit rate, and cost-per-answer
  to any hiring reviewer browsing the site, backed by a metrics endpoint or a KV-aggregated
  source. The on-page panel is the deliverable; an endpoint alone is not reviewer-visible.
...
| AI Gateway migration (1a) changes the `usage` data shape and silently breaks the budget cap or cache metric. | 1a includes an explicit verification step: assert the Gateway exposes `input`/`cache_read`/`cache_creation` token counts; keep the `budget-cap` behavioral test green; if the shape differs, adapt `settleBudget` in the same PR. |
...
1. `/api/ask` runs through Vercel AI Gateway with the budget cap and cache metric intact; an eval suite exists, runs in CI, and gates on correctness + jailbreak resistance; eval/cost/cache metrics are visible on a surface a hiring reviewer can see; `/.well-known/agent.json` and an MCP server are live.
