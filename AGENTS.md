...
| **Before `speckit-plan` on any spec** | **`thinking-inversion` — enumerate the class-of-bugs the implementation introduces; each becomes an explicit plan task, not a claude-review finding** |
...
| After `speckit-plan` produces output for tasks with >5 steps | `thinking-pre-mortem` — run on the plan tasks themselves, not the feature |
...
- **Every plan must include a failure-mode checklist.** Run `thinking-inversion` before `speckit-plan` on any task. Each bug class becomes an explicit plan task — not a claude-review finding after the fact.
