...
| 13-17 | `bash-guard.sh` used raw-substring greps, so it both (a) false-blocked a dangerous string appearing only inside a quoted argument like a commit message (13, hit live), and (b) was evaded by shell-tokenization-equivalent forms — all **verified** live once the extraction fix made the guard functional: quoted `gh "pr" merge`, double-space `gh pr  merge` (14), quoted `git push --"force" origin main` (15), chained `cd x && npm install` past the `^npm` anchor (16), quoted `git add "-A"` / `git add "."` (17). Mitigating context: `bash-guard` is a cooperative-agent guardrail, and the irreversible ops it names are *also* gated server-side (GitHub branch protection blocks force-push/direct-push to `main`; `gh pr merge` is owner-only) — real blast radius is lower than "sole gate". **Fixed here:** rewrote detection to `shlex`-tokenize and match at command position (per shell segment), with a coarse fail-closed grep fallback only when python3 is unavailable, plus 12 regression tests covering every bypass + the commit-message non-block | security-hardening | yes | done |
