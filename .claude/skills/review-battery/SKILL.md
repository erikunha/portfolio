...
redundancy CLAUDE.md forbids everywhere else.
...
| claim-drift | `documentation-engineer` | Read, Grep, Glob, **Write, Edit** | no | **sonnet** | `~/.claude/agents/` |
| gate-robustness | `security-auditor` | Read, Grep, Glob | **no** | opus | `~/.claude/agents/` |
...
Ownership splits the four. The two under `~/.claude/agents/` are editable, so a
...
FINDING
severity: critical
file: .claude/hooks/architect-gate.sh:44
...
Next.js advice. Read CLAUDE.md, STANDARDS.md, and .claude/rules/ before judging,
...
- A comment in `.ts`/`.tsx` application source that is not a machine directive
  ...
  CLAUDE.md's bare-source rule permits nothing else there, a JSDoc contract
...
`.github/workflows`, `.husky`, `scripts/`, and `.claude/hooks` are the usual
...
CLAUDE.md rule, a commit body, or a PR body. The second set is an explicit
