...
- **Review convergence loop — run it on EVERY PR until claude-review approves.** The loop itself (rebase-before-every-push, push-SHA verification, reply-before-resolve, thread-not-timeline, the post-merge transition) lives in `.claude/skills/review-convergence`; `pr-merge-gate` covers the final merge. **`pnpm review:converge` is the loop's state, so ask it rather than remembering:** it names the single next action (reply-then-resolve → add-missing-reply → request-review → converged) and exits non-zero until claude[bot] has posted a non-stale **Approve** on HEAD with every thread resolved and carrying a reply. `.claude/hooks/review-converge-reminder.sh` runs it automatically after any `git push` on a branch with an open PR, which is the moment the loop must re-enter and the moment it gets skipped. The hook is advisory by construction (PostToolUse cannot un-push, and `pnpm ready-to-merge` already gates the merge); its job is to make "the loop is not done" a line in the terminal instead of something to remember.
