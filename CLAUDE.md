...
- **Review your own diff before opening the PR.** Read `git diff origin/main...HEAD` end to end and invoke `pr-review-toolkit:code-reviewer` against it (`review-pr` is a slash command, not an agent). Fix every Critical/Important finding before `gh pr create`. There is no mechanical gate on this since 2026-07-27: the review battery, its findings ledger, the dispatch stamp and the pre-push stamp check were all removed, so this is discipline, not enforcement.
...
- **Drive the PR to green yourself.** Rebase before every push, verify the pushed SHA landed, reply in a thread citing the fix SHA before resolving it, and re-read CI. There is no AI reviewer on PRs since 2026-07-27, so nothing will tell you a finding was missed.
